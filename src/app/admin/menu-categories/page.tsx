...
import { toggleMenuCategoryActive } from "@/app/admin/menu-categories/actions";
...
/**
 * Menu categories management list.
 *
 * A Server Component with no client JavaScript at all. Status toggling is a `<form>` posting
 * to a Server Action, and edit and delete are links, so the whole screen is HTML the browser
 * already knows how to operate - keyboard, screen reader, and no-JS included.
 *
...
export default async function AdminMenuCategoriesPage({ searchParams }: MenuCategoriesPageProps) {
  await requireAdmin();

  const [locale, params, categoriesResult, parentsResult] = await Promise.all([
    getAdminLocale(),
    searchParams,
    listAdminMenuCategories(),
    listAdminCategoryParentGroups(),
  ...
  const parentOf = (mainCategoryId: string) => {
    const parent = parentLookup.get(mainCategoryId);
    return {
      mainCategoryName: parent?.mainCategoryName ?? t("unknownMainCategory"),
      menuName: parent?.menuName ?? t("unknownMenu"),
    };
  };
    ...
    <main className="py-section">
      <Container>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/admin/menu-categories/new" className={buttonStyles()}>
            {t("newCategory")}
          </Link>
        </div>
            ...
            <>
              ...
              <ul className="flex flex-col gap-4 lg:hidden">
                {categories.map((category) => {
                  const parent = parentOf(category.main_category_id);
                    ...
                    {categories.map((category) => {
                      ...
                      return (
                        <tr key={category.id} className="border-b border-border align-top">
                          ...
                          <td lang="ar" dir="rtl" className="py-4 pe-4">
                            {parent.mainCategoryName}
                          </td>
 ...
 * POST, not a link a crawler or a prefetch could follow. Delete is a link to a confirmation
 * page rather than a form, so nothing destructive happens on one click.
 */
function MenuCategoryRowActions({
  id,
  isActive,
  labels,
}: {
  ...
  return (
    <>
      <Link
        href={`/admin/menu-categories/${id}/edit`}
        className={buttonStyles({ variant: "outline", size: "sm" })}
      >
        {labels.edit}
      </Link>

      <form action={toggleMenuCategoryActive}>
      ...
      <Link
        href={`/admin/menu-categories/${id}/delete`}
