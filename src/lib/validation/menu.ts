...
/**
 * Validation for `public.menus`, shared by the client hint and the server action.
 *
 ...
 *
 * `venue_id` is checked for two things: that it is a syntactically valid UUID, and that it
 * names a venue that actually exists. The second check cannot live here - existence is a
...
export const MENU_FIELD_ERROR = {
  ...
  venueInvalid: "venueInvalid",
  /** Referenced by the action when no venue with that id is visible to the admin. */
...
export type MenuFieldError = (typeof MENU_FIELD_ERROR)[keyof typeof MENU_FIELD_ERROR];
...
export const menuInputSchema = z.object({
  name: z
    ...
    .refine((value) => value.length > 0, { message: MENU_FIELD_ERROR.nameRequired })
    .refine((value) => value.length <= NAME_MAX_LENGTH, {
      message: MENU_FIELD_ERROR.nameTooLong,
    }),

  slug: z
    .string()
    .transform((value) => value.trim().toLowerCase())
    .refine((value) => value.length > 0, { message: MENU_FIELD_ERROR.slugRequired })
    .refine((value) => value.length <= SLUG_MAX_LENGTH, {
      message: MENU_FIELD_ERROR.slugTooLong,
    })
    .refine((value) => SLUG_PATTERN.test(value), {
      message: MENU_FIELD_ERROR.slugInvalid,
    }),

  /**
   * Required, a valid UUID, and - separately - must name an existing venue. The empty
   ...
   * submits real ids. The database's `menus_venue_id_fkey` is the backstop that makes the
   * whole chain safe against a venue disappearing mid-edit.
   */
  venueId: z
    .string()
    .trim()
    .refine((value) => value.length > 0, { message: MENU_FIELD_ERROR.venueRequired })
    .pipe(z.uuid({ message: MENU_FIELD_ERROR.venueInvalid })),
  ...
  description: z
    ...
    .refine((value) => value === null || value.length <= DESCRIPTION_MAX_LENGTH, {
      message: MENU_FIELD_ERROR.descriptionTooLong,
    }),
  ...
  displayOrder: z
    ...
    .refine((value) => /^\d+$/.test(value), {
      ...
      message: MENU_FIELD_ERROR.displayOrderInvalid,
    })
    .transform((value) => Number(value))
    .refine((value) => Number.isInteger(value) && value >= 0, {
      message: MENU_FIELD_ERROR.displayOrderNegative,
    })
    .refine((value) => value <= DISPLAY_ORDER_MAX, {
      message: MENU_FIELD_ERROR.displayOrderTooLarge,
    }),
...
export type MenuFieldErrors = Partial<Record<keyof MenuInput, MenuFieldError>>;

/**
 * Parses `FormData` into a validated menu, or into per-field error keys.
 *
...
export function parseMenuForm(
  formData: FormData,
): { ok: true; data: MenuInput } | { ok: false; errors: MenuFieldErrors } {
  const raw = {
  ...
  const result = menuInputSchema.safeParse(raw);
  ...
  for (const issue of result.error.issues) {
    ...
    if (typeof field === "string" && !(field in errors)) {
      errors[field as keyof MenuInput] = issue.message as MenuFieldError;
    }
  }
